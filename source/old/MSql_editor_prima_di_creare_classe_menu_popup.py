...
class MSql_win2_class(QtWidgets.QMainWindow, Ui_MSql_win2):
    ...
    def slot_click_colonna_risultati(self, index):       
        ...
        v_campo_input = QLineEdit()
        v_campo_input.setPlaceholderText('where...')
        v_campo_input.editingFinished.connect(self.slot_where_popup)
        v_action = QWidgetAction(self.o_table_popup)
        v_action.setDefaultWidget(v_campo_input)        
    ...
    def slot_where_popup(self):
        ...
        # controllo se nell'attuale select è già presente una where
        v_where_found = self.v_select_corrente.rfind('WHERE')
        if v_where_found != -1:
            v_new_select = self.v_select_corrente[0:v_where_found+5] + ' (UPPER(' + v_header_item.text() + ') LIKE ' + "'%'"
        else:
            v_new_select = self.v_select_corrente + ' (UPPER(' + v_header_item.text() + ') LIKE ' + "'%'"
